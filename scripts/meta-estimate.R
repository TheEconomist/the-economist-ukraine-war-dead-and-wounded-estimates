...
tests <- !Sys.Date() == as.Date('2026-06-22)
...
iso_week_start <- function(year, week) {
  jan4 <- ymd(paste0(year, "-01-04"))
  jan4 - days(wday(jan4, week_start = 1) - 1) + weeks(week - 1)
}

deaths_via_meduza <- read_csv('source-data/deaths-and-casualties-data/meduza_2026_weekly.csv') %>%
  mutate(
    date = iso_week_start(Year, Week),
    date = if_else(date == as.Date('2022-02-21'), invasion_start, date),
    estimate = EDAssembled,
    estimate_low = EDAssembledLow,
    estimate_high = EDAssembledHigh
  ) %>%
  filter(
    !is.na(estimate),
  ...
  mutate(
    ...
    estimate_low = cumsum(estimate_low),
    estimate_high = cumsum(estimate_high),
    country = 'russia',
    source = 'Meduza'
  )

deaths_via_meduza$source_id <- paste0('deaths_via_meduza_assembled_', seq_len(nrow(deaths_via_meduza)))

# Inspect that this works
ggplot(deaths_via_meduza, aes(x=date, y=estimate))+
  geom_line()+
  geom_line(aes(y=estimate_low))+
  geom_line(aes(y=estimate_high))+
  labs(title = 'Check of Meduza data merge (EDAssembled cumulative, excluding incomplete H2 2025)', x='', y='')
...
deaths_cumulative <- deaths_cumulative %>%
  filter(!grepl('^Mediazona; Meduza', source))
